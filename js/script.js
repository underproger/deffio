document.addEventListener('DOMContentLoaded', function () {
    ...
    var bannerWalletTitle = document.querySelector('.banner-wallet__title');

    if (bannerWalletTitle) {
        animateTitle(bannerWalletTitle);
    }
    // END | Animation Title
    ...
    const manualWalletSection = document.querySelector('.manual-wallet');

    if (manualWalletSection) {
        const tl = gsap.timeline({});

        tl.fromTo('.manual-wallet-bg__wrapper', { height: "0%" }, { height: "100%", delay: 0 }, 0);

        ScrollTrigger.create({
            trigger: '.manual-wallet-bg',
            start: "100px bottom",
            end: () => `+=${document.querySelector(".manual-wallet-bg").offsetHeight}`,
            animation: tl,
            scrub: 1,
            pinSpacing: false,
        })
        ...
        tl2.fromTo(manualWalletSection, { positon: "fixed", bottom: 0 }, {}, 0);

        ScrollTrigger.create({
            trigger: manualWalletSection,
            start: "bottom bottom",
            end: "+=100%",
            animation: tl2,
            scrub: 1,
